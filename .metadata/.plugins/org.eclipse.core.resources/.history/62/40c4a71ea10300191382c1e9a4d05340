...
public class GeneratePromela{
	public static void main(String args[]) throws FileNotFoundException, UnsupportedEncodingException {
		...
		System.out.println("Insira os valores possiveis dos custos dos contratos (Nada superior a 254 nem inferior a 0): ");
		String StringCustos = reader.next();
		LinkedList<Integer> Custos = new LinkedList<Integer>();
		String[] split = StringCustos.split(",");
		...
		if ( i > 0){
			...
			writer.printf("byte paths[%d]; %n",numvertex);// array of paths 
			writer.printf("byte costs[%d]; %n", numvertex); // costs for adjacent nodes
			// inicializar os custos no valor maximo
			for(int p = 0; p < numvertex; p++) {
				writer.printf("costs[%d] = 255; %n", p);
			}
					...
					for(int n = 0; n < Custos.size(); n++) {
						...
						for(int p = 0; p < currentPaths.size(); p++) {
							...
							for(int m = 0; m < pathList.size()-1/* -1 para retirar o ultimo no (o repetido)*/; m++) {
								pathInt = pathInt + pathList.get(m)*((int)Math.pow(10,m));
							}
