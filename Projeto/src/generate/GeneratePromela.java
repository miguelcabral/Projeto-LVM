			...
			for( int k = 0; k < children.size(); k++) { 
				...
				if(currentChild == 0) {
					...
					for(int n = 0; n < Custos.size(); n++) {
						System.out.printf("Insira o custo de %d aceder o target usando %d quando o custo para este ultimo e %d. %n", i,currentChild, Custos.get(n));
						...
						writer.println(" 		else ->");
						writer.println(" 		if");
						writer.printf(" 		:: x == %d -> costs[%d] = %d;",oldcost,currentChild,newcost);
						// Verificar se o minimo do array costs foi alterado
						writer.println(" 			if");
